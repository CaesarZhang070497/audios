...
class TrainDataset(Dataset):
    def __init__(self, params):
        ...
        with open(params['train_manifest'], 'r') as f:
            ...
            for content in reader:
                if(len(content)==0):
                    break;
                mix_path, vocal_path, instrumental_path, duration, samplerate = content
                duration = float(duration)
                for k in range(int(duration)):
                    if (k+self.chunk_duration>duration):
                        break;
                    start_time = k
    ...
    def __getitem__(self, chunk_id):
        ...
        mix_audio = torch.tensor(mix_audio)
        vocal_audio = torch.tensor(vocal_audio)
        instru_audio = torch.tensor(instru_audio)
        ### resample ###
